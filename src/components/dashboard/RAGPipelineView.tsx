  ...
  useEffect(() => {
    ...
    if (status === 'scanning') {
      timer = setTimeout(() => {
        ...
        addLog('✔️ fatwa_dsn_19_qardh.pdf terbaca.');
        addLog('✔️ fatwa_dsn_112_wakalah.pdf terbaca.');
        addLog('✔️ psaak_101_penyajian_keuangan.pdf terbaca.');
        setStatus('chunking');
      }, 1500);
      ...
      <div style={{ background: 'rgba(4, 49, 33, 0.85)', backdropFilter: 'blur(16px)', padding: '40px', borderRadius: '32px', border: '3px solid #cca334', boxShadow: '0 30px 60px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          <div style={{ flexGrow: 1, position: 'relative' }}>
            <span style={{ position: 'absolute', left: '20px', top: '50%', transform: 'translateY(-50%)', color: '#f3c653', fontWeight: 900, fontSize: '18px' }}>📁</span>
            <input 
              type="text" 
              value={folderPath} 
