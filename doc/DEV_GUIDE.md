...
- **Sidebar & Navigasi:** Gunakan komponen `Sidebar` yang mendukung *state management* `isOpen`. Pastikan konten utama menggunakan margin dinamis atau transisi yang halus agar tidak menutupi (*overlap*) elemen navigasi. Pada area Sidebar Staff, pastikan `ThemeToggle` memiliki margin atas yang cukup agar tidak bertumpukan dengan tombol tutup (✕). Pada portal anggota, tombol `ThemeToggle` diletakkan terpusat pada header atas bersandingan dengan lencana sinkronisasi aktif.
