...
- **Optimasi Window Konteks RAG & Pencegahan Timeout**: Ambang batas Unified Context dibatasi ke 120.000 karakter (~30.000 token). Di atas batas ini, RAG otomatis beralih ke similarity search berdensitas tinggi (top 30 chunk) demi memangkas latensi API Next.js ke 2-4 detik dan mencegah kegagalan timeout koneksi.
- **Peningkatan Kapasitas Output Chatbot**: Batas output token (`maxOutputTokens`) dinaikkan menjadi 4096 token agar respons hukum syariah yang mendalam tidak terpotong.
- **Penyelarasan Rendering UI (Line-by-line Chatbot)**: Parser UI chatbot membaca baris-demi-baris secara runut untuk memastikan headings, teks paragraf, dan bullet lists tampil secara berurutan dan tidak teracak.
- **Purifikasi Estetika AI**: Penghapusan total seluruh emoji dari saran prompt dan referensi pustaka/fatwa AI.
